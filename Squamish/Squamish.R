...
ggplot() +
  ...
  geom_sf(
    data = stops_sf,
    aes(size = eigenvector_centrality, fill = eigenvector_centrality),
    shape = 21, color = "black", stroke = 0.3
  ) +
  scale_fill_viridis_c(option = "plasma", name = "Eigenvector\nCentrality") +
  scale_size_continuous(range = c(2, 8), name = "Node Size\n(Centrality)") +
  scale_color_manual(values = route_colors, name = "Route ID") +
  labs(
